 ...
 * @license     Commercial
 *
 * @since  	    Friday, January 09, 2015, 01:09 AM GMT+1
 * @modified    $Date$ $Author$
 * @version     $Id$
 *
 * @category    JavaScript
 * @package     jQuery Embed Dailymotion
*/

(function($) {
    'use strict';

    $.fn.embedDailymotion = function( options ) {
    	 var settings = $.extend({
    ...
    function _embed( $el, options ) {
    	...
    	var urlRegEx	= /^((http:\/\/)?(www\.)?dailymotion\.com\/video\/)([a-z0-9]+)(_(.*)?)$/;
    	var matches;
    ...
    function _getId( url ) {
        var regExp	= /^((http:\/\/)?(www\.)?dailymotion\.com\/video\/)([a-z0-9]+)(_(.*)?)$/;
    ...
    // Embed any Dailymotion URLs that are 
    ...
    function _embedUrls( matches, $el, options ) {
        ...
        $.each( matches, function( index, value ) {
    		...
    		if( videoId !== null ) {
        		if( videoId.length > 0 ) {
                    var template	= '<iframe width="'+ options.width +'" height="'+ options.height +'" src="//www.dailymotion.com/embed/video/__VIDEO_ID__" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
